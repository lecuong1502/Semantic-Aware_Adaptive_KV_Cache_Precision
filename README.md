...
Requires CUDA 12.x with `nvcc`, a C++17 compiler, CMake ≥ 3.28, Python ≥ 3.10,
and an NVIDIA GPU. The engine links the CUDA **driver** API, not only the
runtime API.

```bash
pip install -e .        # builds the CUDA extension via scikit-build-core
pytest                  # kernel, allocator and gate tests
```

Golden reference tensors are generated offline, in a separate environment that
has PyTorch and the HuggingFace model. They are never generated by the test
suite, and PyTorch is never imported in a process that runs the engine.
