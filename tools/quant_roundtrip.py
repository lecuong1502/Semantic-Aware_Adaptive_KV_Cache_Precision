...
- the error in steps of the group's own scale: the mean, and the largest.
  ...
  cannot resolve a step, which tests/test_quant.py derives.
...
compression ratio must quote. Appends one entry per tier. Refuses a dirty
...
def scales(page: np.ndarray, layout: dict, heads: int, head_dim: int) -> tuple:
    """The page's key scales, (1, heads, head_dim), and value scales,
    (P, heads, 1), shaped to broadcast against its keys and values."""
    def halves(name, n):
        return page[layout[name]:layout[name] + 2 * n].view(np.float16).astype(np.float64)

    return (halves("key_scales", heads * head_dim).reshape(1, heads, head_dim),
            halves("value_scales", P * heads).reshape(P, heads, 1))
...
def summary(x: list[np.ndarray], err: list[np.ndarray], steps: list[np.ndarray]) -> dict:
    ...
    return {"relative_rms": rel, "snr_db": float(-20 * np.log10(rel)),
            ...
            "max_steps": float(steps.max()), "elements": int(x.size)}
    ...
    for name in args.tiers:
        ...
        benchlog.append(
            ...
            config={"reference": "float64, against the fp16 values the cache holds",
                    ...
                    "values": "per (head, token)", "page_tokens": P, "issue": 16},
